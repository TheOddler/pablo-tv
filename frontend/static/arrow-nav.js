(() => {
  ...
  */
  ...
  const getFocusableElements = () => {
    return Array.from(document.querySelectorAll('*')).filter(el => el.tabIndex >= 0);
  };
  ...
  */
  const moveFocus = (moveAngle) => {
    ...
    const somethingSelected = currentElement.tabIndex >= 0;
    const curRect = somethingSelected
      ? currentElement.getBoundingClientRect()
      : new DOMRect(0, 0, window.innerWidth, 0);
    ...
    const focussed = elements.find((el) => {
      el.focus({
...
(() => {
  ...
  /** @type {"mouse" | "touch" | "pen" | "keyboard"} */
  let lastInteraction;
  document.addEventListener('pointerdown', e => {
    lastInteraction = e.pointerType; // "mouse", "touch", or "pen"
  });
